...
export default class DetalleVehiculoScreen extends Component {
    ...
    componentDidMount() {
        this.getInfoHerramienta();
    }
    componentWillUnmount() {
        this.props.navigation.state.params.getAlmacenes();
    }
    getInfoHerramienta = async () => {
        const { accessInfo } = this.props.screenProps;
        var formData = new FormData();
        const { idHerramienta } = this.props.navigation.state.params
        formData.append('id', idHerramienta);
        fetch(URLBase + '/alm/getInfoHerramienta', {
            method: 'POST',
            headers: {
            ...
            .then((responseJson) => {
                ...
                if (responseJson.success && responseJson.success == '200') {
                  this.setState({
                        isLoading: false,
                        herramienta: responseJson.herramienta
                    });
                } else {
                    Alert.alert('Error al consultar de la herramienta');
                }
            })
            .catch((error) => {
    ...
    entregarHerramienta = async () => {
        const { accessInfo } = this.props.screenProps;
        var formData = new FormData();
        const { idHerramienta, idProyecto } = this.props.navigation.state.params
        formData.append('id', idHerramienta);
        formData.append('proyecto_id', idProyecto);
        formData.append('cantidad',this.state.cantidad);
        fetch(URLBase + '/alm/entregaHerramienta', {
            method: 'POST',
            headers: {
                Authorization: 'Bearer ' + accessInfo.access_token,
                Accept: 'application/json',
            ...
            .then((responseJson) => {
                ...
                if (responseJson.success && responseJson.success == '200') {
                   this.setState({
                       ...
                       cantidadHer: parseInt(this.state.cantidadHer) - parseInt(this.state.cantidad)
                   });
                } else {
                    Alert.alert('Error al entregar la herramienta')
                }
            })
            .catch((error) => {
                Alert.alert('Error', error);
    ...
    render() {
        const { herramienta, cantidadHer } = this.state;
        return (
            <View style={SharedStyles.container}>
                ...
                {this.state.isLoading == true ? <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    <ActivityIndicator size="large" color={Colors.primary} />
                        ...
                        { herramienta.foto == '' || herramienta.foto == null || herramienta.foto == 'default.jpg' ? <Image
                            source={require('../../assets/images/defaultImage.png')}
                            style={styles.imageStyle} /> : <Image
                                source={{
                                    uri: URLImage + herramienta.foto,
                                }}
                        ...
                        <View style={{ marginTop: 20 }}>
                            ...
                            <View style={styles.filaStyle}>
                                <View style={styles.descripcionContainer}>
                                    ...
                                    <AvenirBook style={styles.text}>
                                        {herramienta.placas === null ? '-------' : herramienta.placas}
                                    </AvenirBook>
                                </View>
                                <View style={styles.descripcionContainer}>
                                    <AvenirHeavy style={styles.text}>
                            ...
                            <View style={styles.filaStyle}>
                                <View style={styles.descripcionContainer}>
                                    ...
                                    <AvenirBook style={styles.text}>
                                        {herramienta.ultimo_servicio==null ? '-------' : herramienta.ultimo_servicio }
                                    </AvenirBook>
                                </View>
                                <View style={styles.descripcionContainer}>
                                    <AvenirHeavy style={styles.text}>
                                    ...
                                    <AvenirBook style={styles.text}>
                                        {herramienta.proximo_servicio==null ? '-------' : herramienta.proximo_servicio}
                                    </AvenirBook>
                                </View>
                            </View>
                            <AvenirHeavy style={styles.text}>
                ...
                <AwesomeAlert
                    ...
                    title={'Seleccione la cantidad de herramientas a devolver'}
